locals {
  app_name                  = "anomaly-detector-mapper"
  config_file_path          = "${path.module}/templates/anomaly-detector-mapper_conf.tpl"
  deployment_yaml_file_path = "${path.module}/templates/deployment_yaml.tpl"
  count                     = "${var.enabled?1:0}"
  checksum                  = "${sha1("${data.template_file.config_data.rendered}")}"
  configmap_name            = "anomaly-detector-mapper-${local.checksum}"
}
...
resource "kubernetes_config_map" "haystack-config" {
  ...
  data {
    "anomaly-detector-mapper.conf" = "${data.template_file.config_data.rendered}"
  }
