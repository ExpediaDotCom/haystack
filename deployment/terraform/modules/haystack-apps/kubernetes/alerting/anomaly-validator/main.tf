locals {
  ...
  config_file_path          = "${path.module}/templates/anomaly-validator_conf.tpl"
  deployment_yaml_file_path = "${path.module}/templates/deployment_yaml.tpl"
  count                     = "${var.enabled?1:0}"
  checksum                  = "${sha1("${data.template_file.config_data.rendered}")}"
  configmap_name            = "anomaly-validator-${local.checksum}"
}
...
resource "kubernetes_config_map" "haystack-config" {
  ...
  data {
    "anomaly-validator.conf" = "${data.template_file.config_data.rendered}"
  }
